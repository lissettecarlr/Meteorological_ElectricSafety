...
void TIM2_IRQHandler(void)      //----TIM4 IRQ------// 
{
 ...
 {
		TIM_ClearITPendingBit(TIM2, TIM_IT_CC1);//清除中断处理位
		Timer2_CC1_IRQ();
 }
if(TIM_GetITStatus(TIM2, TIM_FLAG_Update) != RESET)//捕获1发生捕获事件 
{
...
void TIM3_IRQHandler(void)      //----TIM4 IRQ------// 
{
 ...
 {
		TIM_ClearITPendingBit(TIM3, TIM_IT_CC1);//清除中断处理位
		Timer3_CC1_IRQ();
 }
if(TIM_GetITStatus(TIM3, TIM_FLAG_Update) != RESET)//捕获1发生捕获事件 
{
